...
hermandades.getByField = function(field, value) {
  for (i = 0; i < this.length; i++) {
    //forEach no interrumpe con return
    if (
      this[i][field] &&
      this[i][field] != null &&
      this[i][field].toString().toUpperCase() === value.toString().toUpperCase()
    )
      return this[i]
  }
  return null
  ...
  return getInfo(getPasos)
    .done(function(data) {
      ...
      $.each(pasos, function(i, paso) {
        option = $(
          '<option value=' +
            paso.codigo_toponimo +
            '>' +
            paso.nombre_toponimo +
            '</option>'
        )
...
function cargarCamino(idHermandad) {
  ...
  return getInfo(getCamino + idHermandad)
    .done(function(data) {
      listCamino.empty()
      $('#msjCamino').hide()
      $.each(data.pasos, function(i, paso) {
        ...
        if (ul.length == 1) {
          ...
          ul = $(
            "<ul data-role='listview' id='" + paso.codigo_fecha + "'></ul>"
          )
          div.append(ul)
        }
        ...
        toponimo = {
          ...
          topoY: paso.y,
          topoNombre:
            topoNombre.replace('Paso', 'Paso por') + ' (' + texto_fecha + ')',
        ...
        li = $(
          '<li>' +
            topoNombre.replace('Paso', 'Paso por') +
            "<p class='ui-li-aside'><strong> Horario previsto: " +
            texto_fecha[0] +
            '</strong></p></li>'
        )
        ul.append(li)
  ...
  return getInfo(getHermandades, {
    ...
    .done(function(data) {
      ...
      $.each(data.hermandades, function(i, hermandad) {
        ...
        if (gps > 0) {
          li = $(
            "<li><a href='javascript:pintarMovimientoDiario(" +
              JSON.stringify(hermandad) +
              ',' +
              idDia +
              ")' class='ui-btn ui-btn-icon-right ui-icon-eye'>" +
              hermandad.nombre_largo.substr(0, gps).trim() +
              '</a></li>'
          )
...
function cargarFechasHermandad(idHermandad) {
  return getInfo(getDias + idHermandad)
    .done(function(data) {
      $('#dropDiaRuta').empty()
...
function pintarRuta(hermandad, dia) {
  ...
  getInfo(getRutas + hermandad, filtro)
    .done(function(data) {
      ...
      if (features.length > 0) {
        ...
        showDialog(
          'El trayecto seleccionado no tiene elementos',
          'INFORMACIÓN',
          'warning'
        )
      }
...
function pintarMovimientoDiario(hermandad, dia, jornada) {
  ...
  getInfo(getRutas + hermandad.codigo_hermandad, {
    ...
    .done(function(data) {
      ...
      features = geoJSONformat.readFeatures(data)
      if (features.length > 0) {
        $('#mapaDiario .subheader').text(
          hermandad.nombre + ' - ' + $('#dropDiaDiario option:selected').text()
        )
...
function pintarToponimo(data) {
  if (mapajsTopo === undefined) {
    ...
    mapajsTopo
      .setCenter(data.topoX + ',' + data.topoY + '*true')
      .setZoom(zoomToPoint)
  }
  lyGPS = getLayerGPS()
...
function updateLastPos() {
  ...
  return getInfo(getGPS, filtro, false)
    .done(function(data) {
      vectorSourceGPS.clear()
      vectorSourceGPS.addFeatures(
...
function updateAvisos() {
  ...
  return getInfo(getAvisos, filtro, false)
    .done(function(data) {
...
function pintarGPS(hermandad) {
  if (hermandad != null) {
    //por si se quiere sólo pintar una hermandad
  ...
  bbox =
    vectorSourceGPS.getFeatures().length > 0
      ? vectorSourceGPS.getExtent()
      : bboxContext
        ...
        new ol.style.Style({
          ...
          text: new ol.style.Text({
            text:
              feature.get('name') +
              '\n\r' +
              formatDate(new Date(feature.get('ts'))),
            font: 'bold 9px arial',
            offsetY: -18,
            fill: new ol.style.Fill({
              color: '#000',
...
function bindEvents() {
    ...
    if ($.type(data.toPage) == 'object') {
      switch (data.toPage[0].id) {
        ...
        case 'toponimo':
          ...
          $('#iralli a').attr(
            'onclick',
            `javascript:openUrlExternal('${geolink}');`
          )
  ...
  $('#dropPasos').on('change', function() {
    cargarDiasPaso($(this).val()).done(function() {
  ...
  $('#dropHermandadGps').on('change', function() {
    ...
    if ($(this).val() != 0) {
      ...
      if (h != null && h.lastPos) {
        mapajsGPS
          .setCenter(h.lastPos[0] + ',' + h.lastPos[1])
          .setZoom(zoomToPoint)
      } else {
...
function getGeoLink(coords, label) {
  if (window.isApp) {
    if (window.iOS) return `maps://?ll=${coords}&q=${label}`
    else return `geo:${coords}?q=${coords}(${label})`
  } else return `http://maps.google.com?q=${coords}`
}
